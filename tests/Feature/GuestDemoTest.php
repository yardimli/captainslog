...
class GuestDemoTest extends TestCase
{
    ...
    public function test_landing_page_creates_an_isolated_guest_and_eight_real_date_logs(): void
    {
        ...
        $response = $this->get('/')->assertOk()
            ...
            ->assertSee('An ordinary week, all in one place')
            ->assertSee('Desktop activity · 3 apps · 1 hr 25 min')
            ->assertSee('Kindle reading · The Little Prince')
            ->assertSee('images/demo-dog-walk.png', false)
            ->assertSee('images/demo-language-study.png', false)
            ->assertSee('Chrome extension')
            ->assertSee('Windows desktop app')
        ...
        $this->assertSame(3, $guest->demo_seed_version);
        $this->assertDatabaseHas('task_definitions', ['user_id' => $guest->id, 'name' => 'Medication', 'emoji' => '💊']);
        $this->assertDatabaseHas('task_definitions', ['user_id' => $guest->id, 'name' => 'Dog walk', 'emoji' => '🐕']);
        $this->assertDatabaseCount('attachments', 2);
        foreach (['sensor_google_calendar', 'sensor_desktop', 'sensor_browser', 'sensor_mobile_browser', 'sensor_kindle'] as $sourceType) {
            $this->assertDatabaseHas('log_blocks', ['daily_log_id' => $guest->dailyLogs()->whereDate('log_date', today())->value('id'), 'type' => $sourceType]);
        }
        $this->assertDatabaseHas('log_blocks', ['daily_log_id' => $guest->dailyLogs()->whereDate('log_date', today())->value('id'), 'type' => 'media', 'emoji' => '📷']);
    }
    ...
    public function test_guest_cookie_reconnects_to_the_same_workspace_and_actions_are_persistent(): void
    {
        ...
        $task = TaskDefinition::where('user_id', $guest->id)->where('name', 'Dog walk')->firstOrFail();
        $this->withUnencryptedCookie(GuestDemoService::COOKIE, $token)->postJson(route('demo.events.store', [$log, $task]), ['value' => 'Park loop'])->assertCreated()->assertJsonPath('count', 1);
        $this->assertDatabaseHas('task_events', ['daily_log_id' => $log->id, 'selected_value' => 'Park loop']);
    ...
    public function test_demo_image_delete_removes_tracking_but_not_the_shared_asset_or_reseeds_it(): void
    {
        ...
        $this->assertDatabaseMissing('attachments', ['user_id' => $guest->id, 'path' => 'demo-dog-walk.png']);
    }
