<article class="panel group {{ $block->is_hidden ? 'ring-2 ring-amber-400' : '' }}" id="block-{{ $block->id }}">
    ...
    @elseif($block->type === 'sensor_google_calendar')
        <div class="block-google-calendar-description flex flex-wrap items-center gap-2 leading-relaxed" data-block-description><span class="text-xl" data-block-emoji aria-hidden="true">{{ $block->emoji }}</span><span class="inline-flex rounded-lg bg-blue-100 px-2 py-1 text-xs font-bold uppercase text-blue-900 dark:bg-blue-950 dark:text-blue-200" data-block-type-label>Calendar</span>@if($block->is_hidden)<span class="inline-flex rounded-lg bg-amber-100 px-2 py-1 text-xs font-bold uppercase text-amber-800">Hidden</span>@endif<span class="font-semibold">{{ $block->content }}</span>@if(data_get($block->metadata, 'location'))<span class="text-sm text-slate-500">· {{ data_get($block->metadata, 'location') }}</span>@endif</div>
    ...
    @endif
    @foreach($block->type === 'generated_image' ? $block->attachments : collect() as $attachment)
        <div class="block-attachment mt-3 overflow-hidden rounded-xl bg-slate-100 dark:bg-slate-950">
            <div class="block-image-frame group/image relative h-[512px] max-h-[512px]">
                <img src="{{ $attachment->url }}" alt="AI-generated image" class="h-[512px] max-h-[512px] w-full object-contain" loading="lazy">
                <div class="block-image-controls absolute right-3 top-3 flex gap-2 opacity-100 transition sm:opacity-0 sm:group-hover/image:opacity-100 sm:focus-within:opacity-100">
                    <button type="button" class="grid h-10 w-10 place-items-center rounded-xl bg-slate-950/80 text-white shadow-lg backdrop-blur hover:bg-indigo-600" data-image-preview-open data-image-url="{{ $attachment->url }}" data-image-name="{{ $attachment->original_name }}" aria-label="Open generated image" title="Open image"><svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M15 3h6v6M14 10l7-7M21 14v5a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5"/></svg></button>
                    <a href="{{ $attachment->url }}" download="{{ $attachment->original_name }}" class="grid h-10 w-10 place-items-center rounded-xl bg-slate-950/80 text-white shadow-lg backdrop-blur hover:bg-indigo-600" aria-label="Download generated image" title="Download image"><svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M12 3v12M7 10l5 5 5-5M5 21h14"/></svg></a>
                </div>
            </div>
        </div>
    @endforeach
