<x-app-layout>
    <x-slot name="header"><div class="flex flex-wrap items-center gap-3"><h1 class="text-xl font-bold">Calendar</h1><span class="text-sm text-slate-500">{{ $start->format('M j') }} – {{ $end->format('M j, Y') }}</span></div></x-slot>
    <div class="mx-auto max-w-7xl space-y-4 p-4 sm:p-6 lg:p-8">
        <div class="panel flex flex-wrap items-center gap-2">
        ...
        <div class="grid {{ $view === 'day' ? 'grid-cols-1' : 'grid-cols-2 sm:grid-cols-4 lg:grid-cols-7' }} gap-2">
            @foreach($days as $day)
                ...
                <a href="{{ route('logs.show', $day->toDateString()) }}" class="group min-h-32 rounded-2xl border p-3 transition hover:-translate-y-0.5 hover:border-indigo-400 hover:shadow-md {{ $day->isToday() ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-950/50' : 'border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900' }} {{ $view === 'month' && !$day->isSameMonth($focus) ? 'opacity-50' : '' }}">
                    <div class="flex items-center justify-between"><span class="text-xs font-semibold uppercase text-slate-500">{{ $day->format('D') }}</span><span class="grid h-8 w-8 place-items-center rounded-full {{ $day->isToday() ? 'bg-indigo-600 text-white' : '' }}">{{ $day->day }}</span></div>
                    <div class="mt-5 space-y-1 text-xs text-slate-500">
